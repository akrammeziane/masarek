...
import {
  ...
  LayoutDashboard, Building2, Share2, TrendingUp, RefreshCw, BookOpen,
} from 'lucide-react'
    ...
    <div className="flex min-h-screen bg-[#F4F6F9]" dir="rtl">
        ...
        <nav className="flex-1 p-3 space-y-1">
          {NAV_ITEMS.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white/10 transition-colors group ${!sidebarOpen ? 'justify-center' : ''}`}
            >
              <item.icon className="w-4 h-4 shrink-0 text-white/70 group-hover:text-white" />
              {sidebarOpen && <span className="text-sm text-white/80 group-hover:text-white">{item.label}</span>}
            </Link>
          ))}
        </nav>
      ...
      <main className="flex-1 overflow-auto">
        ...
        <div className="p-6 space-y-6 max-w-5xl">
          {/* Welcome */}
          <div>
            <h1 className="text-2xl font-bold text-[#0B2340]">مرحباً {user.name}! 👋</h1>
            <p className="text-gray-500 text-sm mt-1">هذا هو ملخص وضعك الأكاديمي</p>
          </div>

          {/* No profile yet — prompt to go to settings */}
          {!profile && (
            <div className="rounded-xl p-6 bg-white border-2 border-dashed border-[#C8A84B]/40 text-center">
              ...
              <Link href="/settings">
                <button className="
                  ...
                  shadow-md shadow-[#006233]/20 hover:shadow-lg hover:shadow-[#006233]/30
          ...
          {/* Profile completeness — only when profile exists */}
          {profile && (
            <Card className="border-[#E2E8F0] rounded-xl shadow-sm">
                ...
                {profileCompleteness < 100 && (
                  <p className="text-xs text-gray-400 mt-2">
                    {!profile.assessmentCompleted
                      ? 'أكمل اختبار التوجيه للحصول على توصيات مخصصة'
                      : 'ملفك مكتمل بشكل جيد'}
                  </p>
                )}
              </CardContent>
          ...
          {/* Stats — only when profile exists */}
          {profile && (
              ...
              {[
                {
                  ...
                  color: '#C8A84B', bg: '#C8A84B',
                },
                {
                  label: 'الشعبة',
                  value: profile.bacSeries ? (BAC_SERIES_LABELS[profile.bacSeries] ?? profile.bacSeries) : '—',
                  color: '#006233', bg: '#006233',
                },
                {
                  label: 'التوصيات المتاحة',
                  value: recommendations.length > 0 ? `${recommendations.length} تخصص` : '0',
                  color: '#0B2340', bg: '#0B2340',
                },
                {
                  label: 'تاريخ آخر اختبار',
                  value: assessmentDate ?? '—',
                  color: '#D21034', bg: '#D21034',
                },
              ].map((s) => (
                <Card key={s.label} className="border-[#E2E8F0] rounded-xl shadow-sm overflow-hidden">
                  <div className="h-1.5" style={{ backgroundColor: s.bg }} />
          ...
          {/* AI Recommendation banner — only when profile exists */}
          {profile && (
            profile.assessmentCompleted && topRec ? (
              <div className="rounded-xl p-6 text-white" style={{ background: 'linear-gradient(135deg, #006233, #004d28)' }}>
                <div className="flex items-center gap-5">
                  <div className="w-20 h-20 rounded-full border-4 border-white/30 flex flex-col items-center justify-center shrink-0">
                    <div className="text-2xl font-bold">{Math.round(topRec.matchScore)}%</div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-lg font-bold mb-1 truncate">{topRec.nameAr}</div>
                    <div className="text-white/70 text-xs mb-2">{topRec.nameFr}</div>
                    <div className="flex gap-2 flex-wrap">
                      {topRec.careerPaths.slice(0, 3).map(p => (
                        <span key={p} className="bg-white/15 text-white text-xs px-2.5 py-0.5 rounded-full">{p}</span>
                      ))}
                    </div>
                  </div>
                  <Link href="/results" className="shrink-0">
                    <Button className="bg-white text-[#006233] hover:bg-white/90 font-semibold text-sm">عرض التقرير</Button>
                  </Link>
                </div>
              </div>
            ) : !profile.assessmentCompleted ? (
            ...
            <Card className="border-[#E2E8F0] rounded-xl shadow-sm">
              <CardContent className="p-5">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-[#0B2340]">آخر توصيات الذكاء الاصطناعي</h3>
                  <Link href="/results">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-gray-400 text-xs border-b border-[#E2E8F0]">
                      <th className="text-right pb-2 font-medium">#</th>
                      <th className="text-right pb-2 font-medium">التخصص</th>
                      <th className="text-right pb-2 font-medium">المجال</th>
                      <th className="text-right pb-2 font-medium">التوافق</th>
                      <th className="pb-2" />
                    </tr>
                  </thead>
                  <tbody>
                    {recommendations.slice(0, 5).map((r) => (
                      <tr key={r.id} className="border-b border-[#F4F6F9] last:border-0">
                        <td className="py-2.5 text-gray-400 font-bold text-xs">#{r.rank}</td>
                        <td className="py-2.5 font-medium text-[#0B2340] text-xs md:text-sm">{r.nameAr}</td>
                        <td className="py-2.5">
                          <Badge variant="outline" className="text-xs border-[#E2E8F0] text-gray-500 hidden md:inline-flex">{r.fieldAr}</Badge>
                        </td>
                        <td className="py-2.5">
                          <span className="bg-[#006233]/10 text-[#006233] text-xs font-bold px-2 py-0.5 rounded-full">
                            {Math.round(r.matchScore)}%
                          </span>
                        </td>
                        <td className="py-2.5 text-left">
                          <Link href="/results">
                            <Button variant="ghost" size="sm" className="text-[#006233] text-xs h-7">عرض</Button>
                          </Link>
                        </td>
                      </tr>
                    ))}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <Link key={a.label} href={a.href}>
                <Card className="border-[#E2E8F0] rounded-xl shadow-sm hover:-translate-y-1 hover:border-[#006233]/30 transition-all cursor-pointer h-full">
                  <CardContent className="p-4 flex flex-col items-center text-center gap-2">
                    ...
                    <span className="text-xs font-medium text-[#0B2340]">{a.label}</span>
                  </CardContent>
                </Card>
              </Link>
            ))}
